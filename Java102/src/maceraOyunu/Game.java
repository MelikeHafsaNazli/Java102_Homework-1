...
public class Game {
	...
	public void start() {
		...
		System.out.println(
				"Sayin " + player.getName() + " Bu karanlik ve sisli adaya Hosgeldiniz..Bu adada ki hersey gercek..!");
		player.selectChar();
		...
		while (true) {
			...
			System.out.println(
					"6- Maden         -->  Odul <random nesne para>,  Nehire git, Dikkatli ol yilan cikabilir ");
			...
			case 1:
				if (player.getInventory().getAward()[0].equals("food")
						&& player.getInventory().getAward()[1].equals("water")
						&& player.getInventory().getAward()[2].equals("firewood")) {
				...
				if (awardRand < 15) {
					if (weapRand < 2) {
						...
						if (player.getInventory().getWeapon().getDamage() < Weapon.weapons()[2].getDamage()) {
							...
							System.out.println(
									"Envanterinizdeki esya kazandiginizdan degerli oldugu icin envanterinizdeki ile degistirilmedi");
						}
